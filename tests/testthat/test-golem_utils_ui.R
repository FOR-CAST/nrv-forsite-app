...
test_that("Test list_to_li works", {
  ...
  expect_equal(as.character(list_to_li(c("a", "b"))), "<li>a</li>\n<li>b</li>")
  expect_equal(
...
test_that("Test list_to_p works", {
  ...
  expect_equal(
    as.character(list_to_p(c("This is the first paragraph", "this is the second paragraph"))),
    "<p>This is the first paragraph</p>\n<p>this is the second paragraph</p>"
  )
  expect_equal(
    as.character(list_to_p(
...
test_that("Test tagRemoveAttributes works", {
  ...
  expect_equal(as.character(a_with_tag), '<p src="plop">pouet</p>')

  a_without_tag <- tagRemoveAttributes(a_with_tag, "src")
  expect_s3_class(a_without_tag, "shiny.tag")
  expect_equal(as.character(a_without_tag), "<p>pouet</p>")
})
...
test_that("Test undisplay works", {
  ...
  expect_equal(as.character(a_undisplay), '<p src="plop" style="display: none;">pouet</p>')
  ...
  expect_equal(as.character(c), '<p src="plop" style="some_style">pouet</p>')
...
test_that("Test display works", {
  ...
  expect_equal(as.character(a_display), '<p src="plop" style="">pouet</p>')
})

test_that("Test jq_hide works", {
  expect_s3_class(jq_hide("golem"), "shiny.tag")
  expect_equal(as.character(jq_hide("golem")), "<script>$('#golem').hide()</script>")
})

test_that("Test rep_br works", {
  expect_s3_class(rep_br(5), "html")
  expect_equal(as.character(rep_br(5)), "<br/> <br/> <br/> <br/> <br/>")
})
...
test_that("Test make_action_button works", {
  ...
  button <- make_action_button(tmp_tag, inputId = "mylink")
  expect_s3_class(button, "shiny.tag")
  expect_equal(
    as.character(button),
    '<a href="#" style="color: lightblue;" id="mylink" class="action-button">My super link</a>'
  )
  expect_error(button_2 <- make_action_button(unclass(tmp_tag), inputId = "mylink_2"))
  expect_error(button_3 <- make_action_button(button, inputId = "mylink_3"))
  expect_error(button_4 <- make_action_button(tmp_tag, inputId = NULL))
  tmp_tag_2 <- tmp_tag
  tmp_tag_2$attribs$id <- "id_already_present"
  expect_warning(button_5 <- make_action_button(tmp_tag_2, inputId = "mylink_5"))
  tmp_tag_3 <- tmp_tag
  tmp_tag_3$attribs$class <- "class_already_present"
  button_6 <- make_action_button(tmp_tag_3, inputId = "someID")
})
